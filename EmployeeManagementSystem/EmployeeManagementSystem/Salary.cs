...
namespace EmployeeManagementSystem
{
    ...
    = new SqlConnection(@"Data Source = (LocalDB)\MSSQLLocalDB; AttachDbFilename=C:\Users\maria\Documents\employee.mdf;Integrated Security = True; Connect Timeout = 30");
        ...
        public Salary() 
        {
            InitializeComponent(); //automatically initializes UI elements in the designer

            displayEmployees(); //fetch and display
            disableFields();
        }
        ...
        {
            SalaryData ed = new SalaryData(); //fetch salary data
                    ...
                    if(connect.State == ConnectionState.Closed)
                    {
                        try
                        {
                            ...
                            string updateData = "UPDATE employees SET salary = @salary" +
                                ", update_date = @updateData WHERE employee_id = @employeeID"; //parameterized qqueries to avoid sqlinjection attacks
                            ...
                            using(SqlCommand cmd = new SqlCommand(updateData, connect))
                            {
                                ...
                                cmd.ExecuteNonQuery(); // non query =doesn't return any data but performs the database update.
